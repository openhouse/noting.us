...
export interface PageMetadata {
  notingId: string;
  status: 'seeded' | 'generated_draft' | 'human_edited' | 'published_preview';
...
export interface PermissionsPolicy {
  ...
  sourceQuoteVisibility: 'visible' | 'restricted' | 'redacted';
  retentionPolicy: 'default' | 'short' | 'do_not_store';
...
export interface Workstream {
  ...
  summary: string;
  ownerIds: string[];
  status: 'active' | 'open' | 'paused' | 'later' | 'needs_review';
}
...
export interface PolicyLineageNode {
  id: string;
  yearOrRange: string;
  lane: 'nyc_council' | 'ny_senate' | 'ny_assembly' | 'sbjsa' | 'crs';
  title: string;
  sponsorOrActor?: string;
  sourceCitationIds: string[];
}
export interface LegislativeVehicle {
  ...
  sessionOrYear: string;
  statusAsOfSource?: string;
  sourceCitationIds: string[];
}
